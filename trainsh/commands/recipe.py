...
def cmd_show(args: List[str]) -> None:
    """Show recipe details."""
    if not args:
        print("Usage: train recipes show <name>")
        raise SystemExit(1)
...
def cmd_new(args: List[str]) -> None:
    ...
    if not args:
        print("Usage: train recipes new <name> [--template minimal|feature-tour]")
        raise SystemExit(1)

    name = args[0]
    template_name = "feature-tour"
    rest_args = args[1:]
...
def cmd_edit(args: List[str]) -> None:
    ...
    if not args:
        print("Usage: train recipes edit <name>")
        raise SystemExit(1)

    recipe_path = find_recipe(args[0])
    if not recipe_path:
        print(f"Recipe not found: {args[0]}")
        print("Use 'train recipes new' to create one.")
        raise SystemExit(1)
    _open_editor(recipe_path)
...
def cmd_rm(args: List[str]) -> None:
    """Remove a recipe."""
    if not args:
        print("Usage: train recipes rm <name>")
        raise SystemExit(1)
...
def main(args: List[str]) -> Optional[str]:
    """Main entry point for recipes command."""
