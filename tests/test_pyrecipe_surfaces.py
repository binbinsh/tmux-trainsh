...
class PyrecipeMoreSurfacesTests(unittest.TestCase):
    def test_misc_provider_helpers(self):
        ...
        recipe.xcom_push("rows", {"a": 1}, from_var="ROWS", output_var="OUT", task_id="task", run_id="run", dag_id="dag", map_index=2, database="db.sqlite", id="push")
        recipe.xcom_pull("rows", task_ids="a, b", run_id="run", dag_id="dag", map_index=3, include_prior_dates=True, default={"x": 1}, output_var="PULL", decode_json=True, database="db.sqlite", id="pull")
