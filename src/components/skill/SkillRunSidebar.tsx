...
export interface SkillRunSidebarProps {
  executionId: string;
  /** Called when sidebar should be closed/hidden */
  onClose?: () => void;
}

export function SkillRunSidebar({ executionId }: SkillRunSidebarProps) {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const { data: execution, isLoading, isError } = useInteractiveExecution(executionId);
  ...
  // The prompt info comes from execution.pending_input
  ...
  if (isLoading || !execution) {
    return (
      <Card className="h-full min-h-0 overflow-hidden flex flex-col rounded-none">
        <CardContent className="p-0 min-h-0 flex-1 flex items-center justify-center">
  ...
  if (isError) {
    return (
      <Card className="h-full min-h-0 overflow-hidden flex flex-col rounded-none">
        <CardContent className="p-4 min-h-0 flex-1 flex items-center justify-center">
          <div className="text-sm text-destructive">Failed to load execution</div>
        </CardContent>
      </Card>
    );
  }
