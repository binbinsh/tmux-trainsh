...
fn create_remote_name() -> String {
  format!("doppio_{}", uuid::Uuid::new_v4().to_string().replace("-", "")[..8].to_string())
}
...
pub async fn sync_to_remote(
  ...
  emit_progress(app, &session_id, SyncProgress {
  ...
  if let Err(e) = &result {
    return Err(AppError::command(format!("Failed to create rclone remote: {}", e)));
  }
