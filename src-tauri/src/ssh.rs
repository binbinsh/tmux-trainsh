...
impl SshSpec {
        ...
        if let Some(p) = &self.key_path {
            ...
            if !p.trim().is_empty() {
                ...
                let md = std::fs::metadata(p)
                    .map_err(|e| AppError::io(format!("Failed to stat SSH key: {e}")))?;
                if !md.is_file() {
                    return Err(AppError::invalid_input(format!(
                        "SSH key must be a file: {p}"
                    )));
                }
                let mode = md.permissions().mode() & 0o777;
                // OpenSSH may ignore keys if group/other have any permissions.
                if (mode & 0o077) != 0 {
                    std::fs::set_permissions(p, std::fs::Permissions::from_mode(0o600)).map_err(
                        |e| {
                            AppError::io(format!(
                                "SSH key permissions are too open (mode={mode:o}). Fix with: chmod 600 {p}. Error: {e}"
                            ))
                        },
                    )?;
                }
    ...
    fn build_ssh_options(&self, batch_mode: bool) -> Vec<String> {
        let mut args: Vec<String> = vec!["-p".to_string(), self.port.to_string()];
        if batch_mode {
...
pub async fn ensure_bin(name: &str) -> Result<(), AppError> {
    which::which(name)
        .map_err(|_| AppError::command(format!("Required binary not found: {name}")))?;
    Ok(())
}
