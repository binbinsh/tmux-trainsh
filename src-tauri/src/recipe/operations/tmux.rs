...
pub async fn new_session(
    ...
    if !output.status.success() {
        ...
        if !stderr.contains("duplicate session") {
            return Err(AppError::command(format!("Failed to create tmux session: {stderr}")));
        }
    }
...
pub async fn send_keys(
    ...
    let tmux_cmd = format!("tmux send-keys -t {} '{}' Enter", session_name, keys.replace('\'', "'\\''"));
...
pub async fn session_exists(
    ...
    let tmux_cmd = format!("tmux has-session -t {} 2>/dev/null && echo yes || echo no", session_name);
