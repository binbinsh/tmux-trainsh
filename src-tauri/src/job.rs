...
fn parse_excludes(extra_excludes: Option<String>, include_data: bool, include_models: bool, include_dotenv: bool) -> Vec<PathBuf> {
...
fn bundle_project(project_dir: &Path, bundle_path: &Path, excludes: &[PathBuf]) -> Result<(), AppError> {
...
async fn scp_upload(ssh: &SshSpec, local_path: &Path, remote_path: &str) -> Result<(), AppError> {
  ...
  if !local_path.exists() {
    return Err(AppError::invalid_input(format!("Local file not found: {}", local_path.display())));
  }
...
async fn scp_download_dir(ssh: &SshSpec, remote_dir: &str, local_dir: &Path) -> Result<(), AppError> {
  ensure_bin("scp").await?;
  ssh.validate()?;
...
async fn rsync_download_dir(ssh: &SshSpec, remote_dir: &str, local_dir: &Path) -> Result<(), AppError> {
...
pub async fn run_remote_job(
  ...
  let remote_home = ssh_bash(&ssh, "printf %s \"$HOME\"").await?.trim().to_string();
  ...
  let output_dir = match input.remote_output_dir.clone().unwrap_or_default().trim().to_string() {
  ...
  let tmux_session = format!("doppio_{}_{}", input.instance_id, id.chars().take(8).collect::<String>());

  // Create remote dirs.
  ssh_bash(&ssh, &format!("mkdir -p {}", shell_escape::unix::escape(job_dir.clone().into()))).await?;
  ssh_bash(
    &ssh,
    &format!(
      "mkdir -p {} {}",
  ...
  if do_sync {
    let excludes = parse_excludes(input.extra_excludes.clone(), include_data, include_models, include_dotenv);
...
"#,
  ...
  ssh_bash(
    &ssh,
    &format!("chmod +x {}", shell_escape::unix::escape(remote_run.clone().into())),
  )
  .await?;
...
pub async fn tail_logs(ssh: SshSpec, log_path: String, lines: usize) -> Result<Vec<String>, AppError> {
  let out = ssh_bash(
    &ssh,
    &format!(
...
pub async fn get_exit_code(ssh: SshSpec, job_dir: String) -> Result<Option<i32>, AppError> {
  ...
  let v: i32 = s.parse().map_err(|_| AppError::io(format!("Invalid exit_code: {s}")))?;
  Ok(Some(v))
}

pub async fn download_dir(ssh: SshSpec, remote_dir: String, local_dir: String, delete: bool) -> Result<(), AppError> {
